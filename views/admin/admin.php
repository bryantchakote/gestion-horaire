...
if(isset($_SESSION['users-list']) && !empty($_SESSION['users-list'])){
  $studentSelect = $connexion->prepare('Select nom, prenom FROM personnes WHERE id_pers = ' .$_SESSION['users-list']);
  $studentSelect->execute();
  $studentResult = $studentSelect->fetch();
  
  $datesSelect = $connexion->prepare('
    SELECT date_jour FROM instants WHERE id_pers = ' .$_SESSION['users-list']. '
    UNION
    SELECT date_jour FROM absences WHERE id_pers = ' .$_SESSION['users-list']. '
    ORDER BY date_jour
  ');
  
  $hoursSelect->bindParam(':id_pers', $_SESSION['users-list']);
  $absencesSelect->bindParam(':id_pers', $_SESSION['users-list']);
